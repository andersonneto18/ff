...
import { Copy, LogOut, Sparkles, Wallet, Users, TrendingUp, Send } from 'lucide-react'
...
function InfluencerDashboard({ influencer, onLogout }) {
  ...
  const load = useCallback(async () => {
    try {
      const [d, m] = await Promise.all([api('/influencer/dashboard'), api('/influencer/withdrawal-method')])
      setData(d)
      setMethod(m.method)
...
export default function InfluencerPage() {
  ...
  return <InfluencerDashboard influencer={influencer} onLogout={logout} />
}
