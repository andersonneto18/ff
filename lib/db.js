...
const SCHEMA = [
  ...
  // Per-duel chat so the two entrants (and their accepted duo partners) can agree on a time and
  // coordinate before the room-creator side starts the match.
  `CREATE TABLE IF NOT EXISTS tournament_match_messages (
    ...
    message TEXT NOT NULL,
    createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    KEY idx_tmm_match (matchId, createdAt)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  ...
  // Separate identity from `users` on purpose: influencers earn a cut of platform commission,
  // not player balance, and shouldn't be able to log in as a player or vice versa.
  `CREATE TABLE IF NOT EXISTS influencers (
    ...
    banned TINYINT(1) NOT NULL DEFAULT 0,
    payoutFullName VARCHAR(190) NULL,
    payoutType VARCHAR(20) NULL,
    payoutIban VARCHAR(60) NULL,
    payoutMbway VARCHAR(20) NULL,
    createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uniq_influencers_email (email),
    UNIQUE KEY uniq_influencers_referralCode (referralCode)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,

  `CREATE TABLE IF NOT EXISTS influencer_sessions (
    token VARCHAR(120) NOT NULL PRIMARY KEY,
    influencerId VARCHAR(36) NOT NULL,
    createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    KEY idx_influencer_sessions_influencerId (influencerId)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  ...
  `CREATE TABLE IF NOT EXISTS influencer_transactions (
    ...
    amountCents INT NOT NULL,
    roomId VARCHAR(36) NULL,
    referredUserId VARCHAR(36) NULL,
    withdrawalId VARCHAR(36) NULL,
    balance INT NULL,
    description TEXT,
    createdAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    KEY idx_influencer_transactions_influencerId (influencerId)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
  ...
  `CREATE TABLE IF NOT EXISTS influencer_withdrawals (
    ...
    amountCents INT NOT NULL,
    fullName VARCHAR(190) NULL,
    withdrawalType VARCHAR(20) NULL,
    iban VARCHAR(60) NULL,
    mbway VARCHAR(20) NULL,
    status VARCHAR(20) NOT NULL DEFAULT 'PENDENTE',
    rejectionReason TEXT NULL,
    ...
    KEY idx_influencer_withdrawals_influencerId (influencerId)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,

  // One-off milestone bonuses for reaching N *active* (already-wagered) referred players —
  // each milestone can only be claimed once per influencer (see uniq key).
  // Independent of `users`/`influencers` on purpose: gated only by MAINTENANCE_PASSWORD env var,
  // so a developer keeps overview access even if the app is sold and admin credentials change hands.
  ...
  `CREATE TABLE IF NOT EXISTS influencer_milestones (
    ...
    bonusCents INT NOT NULL,
    claimedAt DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    KEY idx_influencer_milestones_influencerId (influencerId),
    UNIQUE KEY uniq_influencer_milestone (influencerId, milestoneCount)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci`,
]
...
const MIGRATIONS = [
  ...
  `ALTER TABLE tournaments ADD COLUMN mode VARCHAR(120) NULL`,
  `ALTER TABLE tournaments ADD COLUMN server VARCHAR(60) NULL`,
  `ALTER TABLE tournaments ADD COLUMN weapons VARCHAR(190) NULL`,
  `ALTER TABLE tournaments ADD COLUMN platform VARCHAR(30) NULL`,
  `ALTER TABLE tournaments ADD COLUMN rules TEXT NULL`,
  `ALTER TABLE tournament_matches ADD COLUMN firstClaimAt DATETIME NULL`,
  `ALTER TABLE rooms ADD COLUMN characters TEXT NULL`,
  `ALTER TABLE tournaments ADD COLUMN characters TEXT NULL`,
  `ALTER TABLE rooms ADD COLUMN pets TEXT NULL`,
  `ALTER TABLE tournaments ADD COLUMN pets TEXT NULL`,
  `ALTER TABLE users ADD COLUMN referredBy VARCHAR(36) NULL`,
  // Duo partner invite for team-labeled (e.g. "2v2") tournaments — one account still pays/registers,
  // the partner is a purely cosmetic link so other entrants can see who's playing with whom.
  `ALTER TABLE tournament_participants ADD COLUMN partnerId VARCHAR(36) NULL`,
  `ALTER TABLE tournament_participants ADD COLUMN partnerStatus VARCHAR(20) NULL`,
  // Room-creator confirms the match is ready to play — win/loss claims are blocked until then
  `ALTER TABLE tournament_matches ADD COLUMN startedAt DATETIME NULL`,
]
...
async function initDb() {
  ...
  const connectionLimit = process.env.MYSQL_CONNECTION_LIMIT ? Number(process.env.MYSQL_CONNECTION_LIMIT) : 2
