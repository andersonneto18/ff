...
class Collection {
  ...
  async updateOne(filter, update) {
    ...
    await pool.query(`UPDATE \`${this.table}\` SET ${sets.join(', ')} WHERE ${clause}`, [...params, ...whereParams])
  }
...
const MIGRATIONS = [
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS stripeAccountId VARCHAR(255) NULL`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS payoutsEnabled TINYINT(1) NOT NULL DEFAULT 0`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS stripeTransferId VARCHAR(255) NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS stripePayoutId VARCHAR(255) NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS failureReason TEXT NULL`,
  `ALTER TABLE withdrawals MODIFY fullName VARCHAR(190) NULL`,
  `ALTER TABLE withdrawals MODIFY iban VARCHAR(60) NULL`,
  `ALTER TABLE reports ADD COLUMN IF NOT EXISTS resolvedWinnerId VARCHAR(36) NULL`,
  `ALTER TABLE transactions ADD COLUMN IF NOT EXISTS stripeAccountId VARCHAR(255) NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS withdrawalType VARCHAR(20) NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS mbway VARCHAR(20) NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS bank VARCHAR(120) NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS notes TEXT NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS paidAt DATETIME NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS processedByName VARCHAR(120) NULL`,
  `ALTER TABLE withdrawals ADD COLUMN IF NOT EXISTS rejectionReason TEXT NULL`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS notificationsEnabled TINYINT(1) NOT NULL DEFAULT 1`,
  `ALTER TABLE users ADD COLUMN IF NOT EXISTS deviceType VARCHAR(20) NULL`,
  `ALTER TABLE push_subscriptions DROP INDEX IF EXISTS uniq_push_subscriptions_endpoint`,
  `ALTER TABLE push_subscriptions ADD COLUMN IF NOT EXISTS endpointHash CHAR(64) NOT NULL DEFAULT ''`,
  `ALTER TABLE push_subscriptions MODIFY COLUMN endpoint TEXT NOT NULL`,
  `ALTER TABLE push_subscriptions ADD UNIQUE KEY IF NOT EXISTS uniq_push_subscriptions_endpointHash (endpointHash)`,
]
